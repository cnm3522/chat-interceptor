...
可以像使用 / 键 输入命令一样，按下按键自动输入前缀
It can automatically input the prefix by pressing a key, just like pressing / to input a command.

默认使用 ’ 键 （单引号键）；The default key is ' (the apostrophe key).
可以在控制菜单调整；It can be configured in the game's controls menu.

兼容 Baritone 的 # 命令系统;Compatible with Baritone's # command system
# 键为白名单不会被拦截；The # key is a whitelist and will not be intercepted.
⚙️ 客户端管理
/chat 命令管理系统;/chat command management system
设置本地保存;Settings saved locally
纯客户端运行Runs purely on the client side
